...
if [ -n "$PIDS" ]
then
  echo "Ordinator is already running in process $PIDS";
  exit 1
fi
...
LOG_FILE=${LOGGING_PATH}/ordinator.out
ERR_FILE=${LOGGING_PATH}/ordinator.err
...
echo "Giving Ordinator $waitTimeout seconds to start successfully"
...
do
  ...
  then
    echo "Ordinator didn't start within $waitTimeout seconds."
    PIDS=$(pgrep java -lf | grep $APP_NAME | cut -d" " -f1);
    if [ -n "$PIDS" ]
	then
	  echo "Killing $PIDS";
...
if [ $retVal -eq 1 ]
then
  echo "Starting Ordinator failed"
else
  echo "Starting Ordinator succeeded"
fi
