...
then
  /bin/echo "Ordinator is not running"
else
  sleepCounter=0
  sleepIncrement=2
  waitTimeOut=600
  ...
  /bin/echo "Ordinator is running, stopping service"
...
then
  mkdir -p $prefixDir
  /usr/sbin/useradd -r -s /sbin/nologin -d $prefixDir -m -c "Ordinator user for the Ordinator service" $APP_NAME 2> /dev/null || :
fi
